...
i72_urban_shr(t_all, j, "rural") = (im_pop_grid(t_all, j) -  im_pop_urban_grid(t_all, j)) / (im_pop_grid(t_all,j) + 0.00000001) ;
 ...
 i72_proc_demand(t_all, j, use, kall) = sum(cell(i,j), (f72_proc_demand("lic", use, kall) 
                                       - (f72_proc_demand("lic", use, kall) - f72_proc_demand("hic", use, kall)) * im_development_state(t_all,i))) ;
